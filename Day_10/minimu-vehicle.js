...
function calculateMinimumVehiclesRequiredToDeliverParcel(givenAddress) {
    ...
    for (let i = 0; i < sortedVehicle.length; i++) {
        ...
        let capacityOfVehicle = sortedVehicle[i].weightCapacity;
        let countOfVehicle = Math.floor(weightOfParcel / capacityOfVehicle);

        if (countOfVehicle > 0) {
            vehicleCount.push({ type: sortedVehicle[i].type, count: countOfVehicle });
    ...
    if (weightOfParcel > 0) {
    const smallestVehicle = sortedVehicle[sortedVehicle.length - 1];
